...
import static uet.oop.bomberman.entities.enemiesG.Kondoria.*;
import static uet.oop.bomberman.entities.enemiesG.Kondoria.checkbomb;
import static uet.oop.bomberman.graphics.Sprite.SCALED_SIZE;

public class Oneal extends Enemy {
    public static boolean checkbombO = false;
    public static int checkTimeBombO = 0;
    public static int timeDirectionO = 0;
    ...
    public void update() {
        if (startCountdown) {
            ...
            if (Math.abs(bX - x) == 0 && (bY - y) < 0) {
                ...
                if (timeDirectionO == checkTimeBombO + 90) {
                    checkbombO = false;
                }
            }
    ...
    public void generateDirection() {
        ...
        if (ai.checkBombss() != 10) {
            direction = ai.checkBombss();
        } else {


        if (timeDirectionO % 31 == 0) {
            xOld = getX();
            yOld = getY();
            if (timeDirectionO % 93 == 0) newDirection = 0;
        }
        if (timeDirectionO % 35 == 0) {
